...
/// Reads blockerRules.json from the shared App Group container and returns it
/// to Safari. The main app writes this file via ContentBlockerService whenever
/// settings change, then calls SFContentBlockerManager.reloadContentBlocker.
class ActionRequestHandler: NSObject, NSExtensionRequestHandling {

    func beginRequest(with context: NSExtensionContext) {
        // Use rules file if it exists, otherwise fall back to an empty ruleset
